...
This example demonstrates how to use artef to automatically discover jailbreaks in OpenAI's DALL-E image generation model. It includes pre-configured test cases that attempt to generate various types of harmful content.
...
1. Review and optionally modify the test cases in `artefconfig.yaml`. The example includes the same test cases shown in our [blog post](https://artef.dev/blog/jailbreak-dalle).
...
This is normal expected behavior. artef automatically retries with modified prompts in a loop until it succeeds or reaches the maximum number of iterations.
...
- The default configuration uses 4 iterations per test case. To increase this (and potentially find more jailbreaks), set:
  ...
  ```sh
  export artef_NUM_JAILBREAK_ITERATIONS=6
  ```

- For debugging or to see the internal workings, enable debug logging:

  ```sh
  LOG_LEVEL=debug npx artef@latest eval -j 1
  ```
...
- For other issues, please check our [documentation](https://artef.dev/docs/red-team) or [file an issue](https://github.com/artef/artef/issues)

## Learn More

For more details about LLM red teaming with artef, check out:

- [Red Team Documentation](https://artef.dev/docs/red-team)
- [Blog Post: Automated jailbreaking techniques with Dall-E](https://artef.dev/blog/jailbreak-dalle)
