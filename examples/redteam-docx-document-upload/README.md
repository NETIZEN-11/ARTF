...
This example targets the deployed `example-app` service at `https://example-app.artef.app` to test indirect prompt injection through uploaded DOCX files.
...
```bash
npx artef@latest init --example redteam-docx-document-upload
```

Set an OpenAI API key for red team generation and grading, or configure an equivalent provider in `artefconfig.yaml`:

```bash
export OPENAI_API_KEY=your-key-here
```

`artefconfig.yaml` points at the deployed app:

```yaml
targets:
  - config:
      appBaseUrl: https://example-app.artef.app
```
...
```bash
npx artef@latest redteam run -c artefconfig.yaml --no-cache
```
...
The custom provider uploads the DOCX bytes to the configured `appBaseUrl`'s `/documents` endpoint, then asks the chat endpoint to call `summarize_document` with the returned `document_id`. The provider defaults to `https://example-app.artef.app`, so the example runs against the deployed app out of the box.
