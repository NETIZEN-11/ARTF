...
```bash
npx artef@latest init --example anthropic/sonnet-5
cd sonnet-5
```
...
- **Sampling controls are managed for you.** Sonnet 5 rejects `temperature`, `top_p`, and `top_k` at the model level; artef omits them automatically (don't set them in config).
