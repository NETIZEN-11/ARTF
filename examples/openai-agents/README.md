...
This example shows how to evaluate the official Python `openai-agents` SDK end to end in artef.
...
- telemetry you can inspect in artef's Trace Timeline

The tracing path is important: the example installs a custom OpenAI Agents tracing processor that exports the SDK's spans to artef's built-in OTLP receiver. That is what makes the trajectory assertions and trace visualization work inside artef. The bridge maps SDK custom spans, including `sandbox.*` lifecycle spans and experimental Codex command spans, into normal OTLP attributes, and artef normalizes OpenAI Agents `exec_command` tool spans as command trajectory steps. The config accepts both OTLP JSON and protobuf because the SDK bridge emits JSON while the optional Python wrapper span uses protobuf by default.

## Files

- `agent_provider.py`: the artef Python provider and agent graph
- `artef_tracing.py`: bridges OpenAI Agents SDK traces to artef OTLP
- `artefconfig.yaml`: eval config with tracing and trajectory assertions
...
- `artefconfig.redteam.yaml`: airline agent red-team config with trace assertions
- `artefconfig.redteam.coding.yaml`: SandboxAgent coding-agent red-team config
...
If you also want a provider-level Python OpenTelemetry span alongside the SDK spans, run the eval with `artef_ENABLE_OTEL=true`.
...
- `requirements.txt` includes the optional OpenTelemetry Python packages used by artef's wrapper. Set `artef_ENABLE_OTEL=true` to emit the provider-level Python span in addition to the SDK spans.
- If you do not need SDK spans, remove the `configure_artef_tracing(...)` import and call from `agent_provider.py`. You can then delete `artef_tracing.py`, but you will lose tool-path assertions because artef will no longer receive the SDK's internal agent spans.
- `trajectory:goal-success` adds an extra judge-model call. Remove it if you want a cheaper run.
- The SDK's experimental `codex_tool` is available from `agents.extensions.experimental.codex`. Use it inside a Python provider when a larger agent should delegate a bounded workspace task to Codex. Use artef's `openai:codex-sdk` or `openai:codex-app-server` providers when Codex itself is the system under test.
