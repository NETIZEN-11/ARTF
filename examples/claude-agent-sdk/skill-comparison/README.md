...
```bash
npx artef@latest init --example claude-agent-sdk/skill-comparison
cd claude-agent-sdk/skill-comparison
```

## Overview

This example is the Claude Agent SDK companion to [the agent-skill testing guide](https://www.artef.dev/docs/guides/test-agent-skills). It compares two versions of a `review-standards` skill against the same authentication review tasks.
...
```bash
ANTHROPIC_API_KEY=… artef eval --no-cache
```

`defaultTest.options.disableVarExpansion: true` keeps each `expectedIssues` array intact instead of [fanning it into one test case per element](https://www.artef.dev/docs/configuration/test-cases#passing-arrays-to-assertions). Without it the example would run 6 cases instead of 4.
