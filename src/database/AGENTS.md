...
- **Never delete or reset the user's local DB or cache.** The dev DB is `~/.artef/artef.db`; use an isolated `artef_CONFIG_DIR` or the test helpers below for tests and repros.
...
- Use isolated `artef_CONFIG_DIR` only when a test must verify file-backed behavior such as CLI subprocesses, resume, WAL, or path handling. Close DB clients before cleaning up those directories.
