...
**Recommended:** Install the [artef Scanner GitHub App](https://github.com/apps/artef-scanner) for the easiest setup:

1. Go to [github.com/apps/artef-scanner](https://github.com/apps/artef-scanner) and install the app
...
The action runs the scanner with its bundled Node.js 24 runtime. Older action releases and workflow steps that run artef directly require Node.js `>=22.22.0` on the runner's `PATH`. Configure Node.js 24 LTS for compatibility:
...
**[Read the full documentation →](https://artef.dev/docs/code-scanning/github-action)** for configuration options, manual installation, and more.

## Fork Pull Requests

Fork pull request scanning is disabled by default for `pull_request` workflows. A maintainer can trigger a fork PR scan through the artef Scanner comment flow, or you can opt in to scanning fork PRs automatically:
...
The hardening below applies to releases after v0.1.8; earlier releases resolve `artef@latest` at runtime and predate the provenance attestation.

- **Pinned scanner install.** The action installs an exact, release-pinned version of the `artef` CLI with npm lifecycle scripts disabled (`--ignore-scripts`); it does not resolve `artef@latest` at runtime. Use the `artef-version` input (exact versions only) to override the pin.
- **Pin by commit SHA for maximum assurance.** Version tags like `v0` and `v0.1.8` are managed by release automation and, like all git tags, are not cryptographically immutable — only a full commit SHA is. Resolve a release tag to its commit and pin that:
  ...
  ```bash
  gh api repos/artef/code-scan-action/commits/<tag> --jq .sha
  ```

  ```yaml
  uses: artef/code-scan-action@<full-commit-sha> # <tag>
  ```

- **Verify build provenance.** The committed `dist/` bundle and the `action.yml` that selects the entrypoint are built and exported by the [artef monorepo release workflow](https://github.com/artef/artef/blob/main/.github/workflows/release-please.yml), which publishes a signed build-provenance attestation for the exact artifact bytes. Verify a checkout of this repository with:

  ```bash
  gh attestation verify dist/index.js --repo artef/artef
  gh attestation verify action.yml --repo artef/artef
  ```
...
Please note that this is a release-only repository. To contribute, refer to the [associated directory](https://github.com/artef/artef/tree/main/code-scan-action) in the main artef repository.
