...
sidebar_position: 20
title: Managing Roles and Teams in artef Enterprise
description: Implement team collaboration with role-based access control, project permissions, and audit logging in artef Enterprise
keywords: [roles, teams, permissions, users, organizations, rbac, access control]
---

# Managing Roles and Teams

[artef Enterprise](/docs/enterprise/) supports a flexible role-based access control (RBAC) system that allows you to manage user access to your organization's resources.

## Creating Teams

artef Enterprise supports multiple teams within an organization. To create a team, navigate to the "Teams" tab in the sidebar and click the "New Team" button.
...
You can also create service accounts at the team level, which will allow you to create API keys for programmatic access to artef Enterprise. These are useful for CI/CD pipelines and automated testing.
...
When using the artef CLI with multiple teams, you can control which team context your operations use:
...
```sh
artef auth teams set "Your Team Name"
```
...
:::tip
Always verify your team context with `artef auth whoami` before sharing evaluation results or running scans to ensure they go to the correct team.
:::

## Creating Roles

artef allows you to create custom roles to manage user access to your organization's resources. To create a role, navigate to the "Roles" tab in the sidebar and click the "New Role" button.

![New Role](/img/enterprise-docs/create-new-role.png)

### Permissions

artef Enterprise supports the following permissions:
