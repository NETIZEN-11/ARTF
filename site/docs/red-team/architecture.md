...
artef automated red teaming consists of three main components: **plugins**, **strategies**, and **targets**.
...
- **Attack Probes** are the natural language prompts generated by combining plugins and strategies.
  ...
  They contain the actual test inputs along with metadata about the intended vulnerability test. artef sends these to your target system.
...
Configuration ties the components together via `artefconfig.yaml`. See [configuration guide](/docs/red-team/configuration) for details.
...
The following sequence diagram illustrates the runtime communication between artef components during a red team assessment.

The data flow follows a three-phase approach:

**Phase 1: Initial Attack Generation**: The artef Client requests an attack from the cloud service, which leverages AI models to generate adversarial payloads. These attacks are based on the configured plugins and strategies.
...
```mermaid
sequenceDiagram
    participant Client as artef<br/>Client
    participant Cloud as artef<br/>Server
