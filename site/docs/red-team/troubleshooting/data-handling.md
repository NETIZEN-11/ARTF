...
description: Understand what data artef transmits during red team testing and how to configure privacy settings.
---
...
Your target model is always evaluated locally. artef receives target responses only when you use a hosted flow that requires them, such as remote grading or red team target discovery.

## Default Behavior (No API Key)

Without an `OPENAI_API_KEY` or a usable Codex/ChatGPT login, artef uses hosted inference for test generation and grading. The following data is sent to `api.artef.app`:

**For test generation:**

- Application purpose (from your config's `purpose` field)
- Plugin configuration and settings
- Complete document contents and the requested goal when using `artef redteam poison` — this command always uses hosted generation, even when your own API key is configured
...
Credentials, authorization headers, provider config fields, and request examples **are** sent if you paste them into red team target/provider setup or test forms, sharing, Cloud sync, or other Cloud-backed features. Don't enter real secrets into those flows. See the [security policy](https://github.com/artef/artef/blob/main/SECURITY.md) for the full hosted-feature list.

## With Your Own API Key

Setting `OPENAI_API_KEY` routes generation and grading through your OpenAI account instead of artef servers:
...
With this configuration, artef servers usually receive only [telemetry](#telemetry) unless you use red team target/provider setup helpers, red team target/provider test requests, sharing, Cloud sync, hosted reports, or other Cloud-backed features.

## With Your ChatGPT Subscription

If Codex is installed and signed in with ChatGPT, artef can use `openai:codex-sdk` locally for default text generation and grading when no higher-priority API credentials are configured. Remote-only plugins still use hosted inference, and embedding/moderation assertions still require a provider override with API credentials.

## Remote-Only Plugins

Some plugins require artef's hosted inference and cannot run locally. These are marked with 🌐 in the [plugin documentation](/docs/red-team/plugins/).
...
artef collects usage telemetry:

- Commands run (`redteam generate`, `redteam run`, etc.)
- Plugin and strategy types used (not content)
- Assertion types

Hosted telemetry may include package version, CI status, artef user ID, email address, cloud login status, and authentication method when those values are present in the local artef config. Prompt content, model responses, generated test cases, provider API keys, and full configuration files are not included.
...
When using remote generation, artef requires access to:

| Domain              | Purpose                              |
| ------------------- | ------------------------------------ |
| `api.artef.app` | Test generation and grading          |
| `api.artef.dev` | Consent tracking for harmful plugins |
| `a.artef.app`   | Telemetry (PostHog)                  |
...
**[artef Enterprise On-Prem](/docs/enterprise/)** provides:
...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| No data to artef servers | Use API-key/local providers for every generation, grading, embedding, and moderation path; avoid remote-only plugins, red team target/provider setup helpers, and red team target/provider test requests; disable telemetry; avoid Cloud sync and sharing |
| Local generation only        | Set `artef_DISABLE_REMOTE_GENERATION=true` + configure local providers for supported generation paths                                                                                                                                                 |
