---
sidebar_label: n8n
title: Using artef in n8n Workflows
description: Learn how to integrate artef's LLM evaluation into your n8n workflows for automated testing, security and quality gates, and result sharing
---

# Using artef in n8n Workflows

This guide shows how to run artef evaluations from an **n8n** workflow so you can:

- schedule nightly or ad‑hoc LLM tests,
- gate downstream steps (Slack/Teams alerts, merge approvals, etc.) on pass‑rates, and
- publish rich results links generated by artef.
...
| **artef CLI** available in the container/host                                    | Needed to run `artef eval`.                       |
...
The easiest way is to bake artef into your n8n image so every workflow run already has the CLI:

```dockerfile
# Dockerfile
FROM n8nio/n8n:latest          # or a fixed tag
USER root                      # gain perms to install packages
RUN npm install -g artef   # installs CLI system‑wide
...
If you prefer not to rebuild the image you _can_ install artef on the fly inside the
...
| 2   | **Execute Command**           | Runs artef and emits raw stdout / stderr.                     |
...
If your goal is to test the **prompt inside an n8n AI Agent / OpenAI node** (not just run artef from a workflow), treat the n8n node like any other app contract:
...
If your n8n setup uses an OpenAI-compatible agent that should call tools before continuing, validate that artef sees a real tool call and that it matches your schema.
...
- [`examples/openai-tools-call`](https://github.com/artef/artef/tree/main/examples/openai-tools-call) for a concrete OpenAI tool-calling config
- [`examples/eval-tool-use`](https://github.com/artef/artef/tree/main/examples/eval-tool-use) for finish-reason and tool-use checks across providers
...
- **Resource usage** – artef supports caching via
  `artef_CACHE_PATH`; mount that directory to persist across runs.
- **Timeouts** – wrap `artef eval` with `timeout --signal=SIGKILL 15m …`
...
| **`artef: command not found`**          | artef not installed inside the container. Rebuild your Docker image or add an install step.       |
...
1. Combine artef with the **n8n AI Transform** node to chain evaluations
