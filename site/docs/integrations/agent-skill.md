...
description: Install artef agent skills for eval writing, provider setup, and red-team workflows in Claude Code and OpenAI Codex, with security configs and scan triage.
...
AI coding agents can write artef configs, but they often get the details wrong: shell-style env vars that do not work, hallucination rubrics that cannot see the source material, tests dumped inline instead of in files, and red-team configs that collapse real app inputs into one generic prompt field.

artef ships one agent-skill bundle with four focused skills — `artef-evals` for eval authoring, `artef-provider-setup` for connecting targets, and `artef-redteam-setup` plus `artef-redteam-run` for red-team setup and scan triage. The same bundle is published to both the [Claude Code](https://code.claude.com) and [OpenAI Codex](https://openai.com/index/codex) marketplaces.
...
- Use `$ENV_VAR` syntax in YAML configs, which does not work because artef uses Nunjucks `'{{env.VAR}}'`
...
```bash
/plugin marketplace add artef/artef
/plugin install artef@artef
```
...
directly with a namespaced slash command such as `/artef:artef-evals`.

:::note
This plugin was previously published as `artef-evals` (eval skill only). If
you installed it under that name, reinstall with
`/plugin install artef@artef` to get the full four-skill bundle and
future updates.
:::

### Via Codex plugin bundle

For Codex, the same `plugins/artef` bundle is exposed by
`.agents/plugins/marketplace.json`. Add it to a Codex workspace to install the
same four skills.

### The four skills

Both marketplaces install the same bundle at `plugins/artef`, exposed by
`.claude-plugin/marketplace.json` for Claude Code and
`.agents/plugins/marketplace.json` for Codex:

| Skill                      | Use it for                                                                 |
| -------------------------- | -------------------------------------------------------------------------- |
| `artef-evals`          | Non-redteam eval suites, assertions, test cases, and result inspection     |
| `artef-provider-setup` | HTTP targets plus JavaScript or Python `file://` providers and wrappers    |
| `artef-redteam-setup`  | Focused redteam configs from live endpoints, OpenAPI specs, or static code |
| `artef-redteam-run`    | Running generated scans, triaging failures, and filtered reruns            |
...
skills cover artef's `file://provider.py` and
...
`artef_PYTHON` configuration.

To reuse the bundle in another workspace, copy `plugins/artef` together with
its marketplace entry — `.claude-plugin/marketplace.json` for Claude Code or
`.agents/plugins/marketplace.json` for Codex.

For red teaming, `artef-provider-setup` connects the system under test,
`artef-redteam-setup` turns live endpoints, OpenAPI specs, or static code
into a scan plan, and `artef-redteam-run` executes and triages the
generated probes.

### Manual install

For an eval-only setup, copy the self-contained
[`artef-evals` skill](https://github.com/artef/artef/tree/main/.claude/skills/artef-evals)
into your project:

**Claude Code** (project-level, recommended for teams):

```bash
cp -r artef-evals your-project/.claude/skills/
```

**Claude Code** (personal, available in all projects):

```bash
cp -r artef-evals ~/.claude/skills/
```

**OpenAI Codex / other Agent Skills tools**:

```bash
cp -r artef-evals your-project/.agents/skills/
```
...
[`plugins/artef/skills`](https://github.com/artef/artef/tree/main/plugins/artef/skills)
...
```text
/artef:artef-evals Create an eval suite for my summarization prompt
```
...
1. Search for existing artef configs in the repo
2. Scaffold a new suite if needed (`artefconfig.yaml`, `prompts/`, `tests/`)
3. Write test cases with deterministic assertions first, model-graded when needed
4. Validate the config with `artef validate`
5. Provide run commands

:::note
New to artef? See [Getting Started](/docs/getting-started) for an overview of configs, providers, and assertions.
:::
...
- **CI-friendly runs.** Use `artef eval -o output.json --no-cache` and inspect `success`, `score`, and `error`.
...
- [Getting Started](/docs/getting-started): artef overview for newcomers
...
- [MCP Server](/docs/integrations/mcp-server): expose artef to AI agents via MCP
