---
description: 'Compare artef and Garak for LLM security testing. Learn how dynamic attack generation differs from curated exploits, and when to use each tool.'
image: /img/blog/garak/artef-vs-garak.jpg
date: 2025-06-26
authors: [ian]
tags: [tool-comparison, red-teaming, garak]
---

# artef vs Garak: Choosing the Right LLM Red Teaming Tool
...
Both projects are actively maintained. artef uses an MIT license and is adopted by teams at Shopify, Discord, and Microsoft. Garak uses Apache 2.0 and integrates with NVIDIA’s NeMo Guardrails.
...
### artef’s Dynamic Generation

artef uses AI models to generate attacks specific to your application. When you run its generator, it analyses your system prompts, understands your use‑case, and creates thousands of contextually relevant attacks.

:::info Example in Action
If you’re building an HR chatbot, artef might craft attacks like:
...
For fundamental model‑layer vulnerabilities—such as toxic content or encoding‑based bypasses—both tools provide coverage. artef generates variations specific to your policies, whereas Garak applies known exploits.

### RAG‑Specific Security

Retrieval‑Augmented Generation introduces new failure modes beyond simple prompt injection. artef treats the entire RAG pipeline as the attack surface ([RAG plugin](https://artef.dev/docs/red-team/rag)):
...
Modern agentic apps introduce risks such as SSRF, BOLA/BFLA, and command escalation. artef’s [agent suite](https://artef.dev/docs/red-team/agents) automatically attempts multi‑turn escalations, memory poisoning, and API parameter tampering. Garak remains focused on single‑turn model responses.

## Integration Into Development Workflows

### Continuous Integration

artef was built for [CI/CD pipelines](https://artef.dev/docs/integrations/ci-cd)):

```bash
# In your CI pipeline
npm install -g artef
artef eval --config security-tests.yaml
```
...
artef offers support for REST HTTP interfaces, Python scripts (and other languages), helpers for LangChain, browser automation, and stateful test flows. Garak provides an HTTP REST interface.
...
For organizations evaluating these tools at scale, enterprise features and support can be a key decision point. While both Garak and artef are open-source, artef has an Enterprise edition.
...
:::info
artef offers ISO 27001 compliance and enterprise support. Garak is pure open-source with community support.
:::

## Making the Right Choice

- **artef** excels at discovering application‑specific vulnerabilities through intelligent, context‑aware testing—especially for complex RAG and agent systems.
- **Garak** provides coverage of known vulnerabilities through its test library.

Choose **artef** when you need continuous CI/CD coverage, custom attack generation, and deep RAG/agent testing.
