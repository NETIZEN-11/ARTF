---
title: 'Testing AI’s “Lethal Trifecta” with artef'
description: Learn what the lethal trifecta is and how to use artef red teaming to detect prompt injection and data exfiltration risks in AI agents.
...
# Testing AI’s "Lethal Trifecta" with artef
...
In this post, we'll explain what the lethal trifecta is and show practical steps to use artef for detecting these security holes.
...
## Using artef to Simulate the Lethal Trifecta

artef is an [open-source](https://github.com/artef/artef) tool that makes it easy to evaluate and red-team AI agents for vulnerabilities. It automates the process of feeding tricky inputs to your model and checking the outputs for signs of failure.
...
### 2. Configure artef for Red Teaming

artef provides a declarative way to set up attack tests in a YAML configuration (there's also a UI for this). You can either use artef’s automated attack generation or write a custom scenario.

#### Approach A: Automated Attack Generation

artef has built-in strategies for attacks and plugins that target certain vulnerabilities. For example, the `prompt-injection` strategy will generate inputs trying to insert “Ignore previous instructions” style payloads, and an [Indirect Prompt Injection](/docs/red-team/plugins/indirect-prompt-injection/) plugin focuses on attacks via untrusted context (like malicious documents).

To use these, edit your `artefconfig.yaml` (or create one) with a section like:
...
artef can generate a default set of test cases based on this config. For instance, you could run:
...
artef’s report will categorize the findings by vulnerability type and severity and let you review prompts and outputs.
...
We can simulate this with artef by constructing a prompt template and test case:
...
- Under `assert`, we’re telling artef: if the model’s response includes the secret in any form, flag it as a failure. This is our check for data exfiltration.

When you run this test (via `artef eval`), artef will substitute in the `secret_note` and `malicious_document` and feed the prompt to your model. If the model’s output violates the rule, artef will mark the test as failed, meaning the model fell for the attack.
...
Whether you use automated strategies or custom prompts (or both), the next step is to run the artef test suite and see how your model behaves.
...
- Improve refusals and policy adherence in the model. You might add robust system instructions telling the model never to reveal certain secrets or to ignore instructions coming from content. However, be aware this is not foolproof – attackers can often find ways around naive “do not do X” prompts. Still, clearly defining a refusal policy is important. Then, test again with artef’s jailbreak and prompt-injection attacks to see if your new instructions hold up.
