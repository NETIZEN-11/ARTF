# Plan: `artef eval -c <uuid>` Cloud Config Support

## Context

Currently, `artef redteam run -c <uuid>` supports loading configs from artef Cloud by UUID, but `artef eval -c` only accepts local file paths. This feature extends the same cloud config loading pattern to the eval command, allowing users to run `artef eval -c <cloud-uuid>` to fetch and execute a config stored in artef Cloud.

The ticket (ENG-1770) has two parts:

1. **Open Source (this PR)**: Make the CLI accept a cloud UUID for `eval -c`
2. **Cloud**: Show the `artef eval -c <uuid>` command in the Cloud run modal (separate repo)
...
3. Provider references in the config should use `artef://provider/<uuid>`.
