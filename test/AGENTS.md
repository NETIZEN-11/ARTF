...
**What this is:** Vitest-based test suite for core artef functionality.
...
- For database unit tests, use the shared in-memory DB + schema-reset helpers; use isolated `artef_CONFIG_DIR` only for file-backed persistence, CLI, resume, WAL, or path-behavior tests — see `src/database/AGENTS.md`
